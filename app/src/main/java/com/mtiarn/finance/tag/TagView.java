...
public class TagView extends RelativeLayout {
    ...
    public void setAlignEnd(boolean alignEnd) {
        mAlignEnd = alignEnd;
    }
	...
	private void initialize(Context ctx, AttributeSet attrs, int defStyle) {
		...
		ViewTreeObserver viewTreeObserber = getViewTreeObserver();
		viewTreeObserber.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
			@Override
	...
	private void drawTags() {
		...
		for (Tag item : mTags) {
            ...
            if (tag.onTagClickListener != null) {
                tagLayout.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View view) {
                        tag.onTagClickListener.onTagClick(tag, position);
                    }
                });
            } else {
                tagLayout.setOnClickListener(new OnClickListener() {
			...
			float tagWidth = tagView.getPaint().measureText(tag.text.toString()) + textPaddingLeft + textPaddingRight;
			...
			if (mWidth <= total + tagWidth + Utils.dipToPx(this.getContext(),Constants.LAYOUT_WIDTH_OFFSET)) {
                ...
                if (listIndex != index_header) {
                    tagParams.addRule(mAlignEnd ? RelativeLayout.LEFT_OF : RelativeLayout.RIGHT_OF, listIndex - 1);
                    if (mAlignEnd) {
                    ...
                    tagParams.addRule(mAlignEnd ? RelativeLayout.ALIGN_PARENT_END : RelativeLayout.ALIGN_PARENT_START, index_header);
                }
    ...
    public void addTags(String[] tags){
        ...
        for(String item:tags){
            Tag tag = new Tag(new SpannableString(item));
            addTag(tag);
        }
    }
