...
import com.mtiarn.finance.classes.SumsByCabbage;
...
public class SumsManager {
    ...
    public static synchronized ListSumsByCabbage formatSums(ListSumsByCabbage listSumsByCabbage,
                                                            HashMap<Long, Cabbage> cabbages,
                                                            boolean isAddStartBalance) {
        CabbageFormatter cf;
        SumsByCabbage sums;
        for (int j = 0; j < listSumsByCabbage.size(); j++) {
            if (!listSumsByCabbage.get(j).isEmpty(isAddStartBalance)) {
                sums = listSumsByCabbage.get(j);
                cf = new CabbageFormatter(cabbages.get(listSumsByCabbage.get(j).getCabbageId()));
                listSumsByCabbage.get(j).setInString(cf.format(sums.getInTrSum()));
                listSumsByCabbage.get(j).setOutString(cf.format(sums.getOutTrSum()));
                if (isAddStartBalance) {
                    listSumsByCabbage.get(j).setTotalString(cf.format(sums.getInTrSum().add(sums.getOutTrSum()).add(sums.getStartBalance())));
                } else {
                    listSumsByCabbage.get(j).setTotalString(cf.format(sums.getInTrSum().add(sums.getOutTrSum())));
                }
            }
        }
    ...
    public static synchronized void updateSummaryTableWithFormattedStrings(Activity activity, TableLayout layoutSumTable,
        ...
        for (int j = 0; j < listSumsByCabbage.size(); j++) {
            if (!listSumsByCabbage.get(j).isEmpty(isAddStartBalance)) {
                ...
                //Income sum
                textView = new TextView(activity);
                setTextViewParams(textView, 1, sums.getInString(), false, textSize1, Gravity.START);
                if (income.compareTo(BigDecimal.ZERO) > 0) {
                    textView.setTextColor(positiveAmountColor);
                } else if (income.compareTo(BigDecimal.ZERO) < 0) {
                    textView.setTextColor(negativeAmountColor);
                ...
                //Expense sum
                textView = new TextView(activity);
                setTextViewParams(textView, 1, sums.getOutString(), false, textSize1, Gravity.CENTER);
                if (expense.compareTo(BigDecimal.ZERO) > 0) {
                    textView.setTextColor(positiveAmountColor);
                } else if (expense.compareTo(BigDecimal.ZERO) < 0) {
                    textView.setTextColor(negativeAmountColor);
                ...
                //Total sum
                textView = new TextView(activity);
                setTextViewParams(textView, 1, sums.getTotalString(), false, textSize1, Gravity.END);
                if (total.compareTo(BigDecimal.ZERO) > 0) {
                    textView.setTextColor(positiveAmountColor);
                } else if (total.compareTo(BigDecimal.ZERO) < 0) {
                    textView.setTextColor(negativeAmountColor);
